...
final class CampaignEditor {
	...
	public function registerPage(): void {
		add_submenu_page(
			null,
			__( 'Campaign Editor', 'wc-campaign' ),
			__( 'Campaign Editor', 'wc-campaign' ),
			'manage_woocommerce',
			self::PAGE_SLUG,
			[ $this, 'render' ]
		...
		if ( 'post-new.php' === $pagenow && PostType::TYPE === sanitize_key( (string) ( $_GET['post_type'] ?? '' ) ) ) {
			$campaignId = wp_insert_post(
				[
					...
					'post_title'  => __( 'Untitled Campaign', 'wc-campaign' ),
	...
	public function filterRowActions( array $actions, \WP_Post $post ): array {
		...
		if ( isset( $actions['edit'] ) ) {
			$actions['edit'] = '<a href="' . esc_url( $this->editorUrl( $post->ID ) ) . '">' . esc_html__( 'Edit Campaign', 'wc-campaign' ) . '</a>';
		}
		return $actions;
	}
	...
	public function render(): void {
		...
		if ( ! $campaign ) {
			wp_die( esc_html__( 'Campaign not found.', 'wc-campaign' ) );
		}
		...
		<div class="wrap woo-campaign-editor-shell">
			<form id="woo-campaign-editor-form" method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>">
				...
				<header class="woo-campaign-editor-topbar">
					<div class="woo-campaign-editor-titlebar">
						<a class="woo-campaign-editor-back" href="<?php echo esc_url( admin_url( 'edit.php?post_type=' . PostType::TYPE ) ); ?>"><span class="dashicons dashicons-arrow-left-alt2"></span><?php esc_html_e( 'Campaigns', 'wc-campaign' ); ?></a>
						<div>
							<div class="woo-campaign-editor-kicker"><?php esc_html_e( 'Campaign', 'wc-campaign' ); ?> · #<?php echo esc_html( (string) $campaignId ); ?></div>
					...
					<div class="woo-campaign-editor-actions">
						<?php if ( 'publish' === $campaign->post_status && ! $archived ) : ?><a class="button" target="_blank" rel="noopener" href="<?php echo esc_url( get_permalink( $campaignId ) ); ?>"><?php esc_html_e( 'Preview', 'wc-campaign' ); ?></a><?php endif; ?>
						<button type="submit" class="button button-primary button-large"><?php esc_html_e( 'Save Campaign', 'wc-campaign' ); ?></button>
					</div>
				</header>

				<?php if ( isset( $_GET['updated'] ) ) : ?><div class="notice notice-success is-dismissible"><p><?php esc_html_e( 'Campaign saved.', 'wc-campaign' ); ?></p></div><?php endif; ?>
				...
				<div class="woo-campaign-editor-layout">
					<main class="woo-campaign-editor-main">
						<section class="woo-campaign-editor-card">
							<div class="woo-campaign-editor-card-heading">
								<div><span class="woo-campaign-editor-eyebrow"><?php esc_html_e( 'Campaign information', 'wc-campaign' ); ?></span><h2><?php esc_html_e( 'Basic information', 'wc-campaign' ); ?></h2></div>
								<p><?php esc_html_e( 'Set the campaign name and URL. Product content is managed in the Section Builder below.', 'wc-campaign' ); ?></p>
							</div>
							<div class="woo-campaign-editor-fields two-col">
								<label><span><?php esc_html_e( 'Campaign name', 'wc-campaign' ); ?></span><input type="text" name="campaign_title" value="<?php echo esc_attr( $campaign->post_title ); ?>" required></label>
								<label><span><?php esc_html_e( 'URL slug', 'wc-campaign' ); ?></span><div class="woo-campaign-slug-field"><code>/campaign/</code><input type="text" name="campaign_slug" value="<?php echo esc_attr( $campaign->post_name ); ?>"></div></label>
							</div>
						</section>

						<section class="woo-campaign-editor-card woo-campaign-content-card" data-campaign-content-card>
							<div class="woo-campaign-editor-card-heading">
								<div><span class="woo-campaign-editor-eyebrow"><?php esc_html_e( 'Campaign content', 'wc-campaign' ); ?></span><h2><?php esc_html_e( 'Campaign content', 'wc-campaign' ); ?></h2></div>
								<p><?php esc_html_e( 'Set campaign images and introduction content.', 'wc-campaign' ); ?></p>
							</div>
							<label class="woo-campaign-editor-field"><span><?php esc_html_e( 'Campaign introduction', 'wc-campaign' ); ?></span><textarea name="campaign_description" rows="5" placeholder="<?php esc_attr_e( 'Describe this campaign, its offer, or important purchase notes.', 'wc-campaign' ); ?>"><?php echo esc_textarea( $campaign->post_content ); ?></textarea></label>
						</section>

						<section class="woo-campaign-editor-card woo-campaign-design-card" data-campaign-design-card>
							<div class="woo-campaign-editor-card-heading">
								<div><span class="woo-campaign-editor-eyebrow"><?php esc_html_e( 'Page display', 'wc-campaign' ); ?></span><h2><?php esc_html_e( 'Campaign Design', 'wc-campaign' ); ?></h2></div>
								<p><?php esc_html_e( 'Fields that are not set inherit from the active theme.', 'wc-campaign' ); ?></p>
							</div>
						...
						<section class="woo-campaign-editor-card woo-campaign-sections-card">
							<div class="woo-campaign-editor-card-heading with-action">
								<div><span class="woo-campaign-editor-eyebrow"><?php esc_html_e( 'Page builder', 'wc-campaign' ); ?></span><h2><?php esc_html_e( 'Product sections', 'wc-campaign' ); ?></h2><p><?php esc_html_e( 'Build the campaign page with sections. Each section can have its own layout, image, copy, and products.', 'wc-campaign' ); ?></p></div>
								<button type="button" class="button button-primary" data-woo-campaign-add-section><span class="dashicons dashicons-plus-alt2"></span><?php esc_html_e( 'Add product section', 'wc-campaign' ); ?></button>
							</div>
					...
					<aside class="woo-campaign-editor-sidebar">
						<section class="woo-campaign-editor-card sticky-card">
							<div class="woo-campaign-editor-card-heading compact"><div><span class="woo-campaign-editor-eyebrow"><?php esc_html_e( 'Lifecycle', 'wc-campaign' ); ?></span><h2><?php esc_html_e( 'Publish settings', 'wc-campaign' ); ?></h2></div></div>
							<label class="woo-campaign-editor-field"><span><?php esc_html_e( 'Status', 'wc-campaign' ); ?></span><select name="campaign_post_status"><option value="draft" <?php selected( $campaign->post_status, 'draft' ); ?>><?php esc_html_e( 'Draft', 'wc-campaign' ); ?></option><option value="publish" <?php selected( $campaign->post_status, 'publish' ); ?>><?php esc_html_e( 'Published', 'wc-campaign' ); ?></option></select></label>
							<label class="woo-campaign-editor-field"><span><?php esc_html_e( 'Start time', 'wc-campaign' ); ?></span><input type="datetime-local" name="campaign_start_at" value="<?php echo esc_attr( $this->formatTimestamp( $start ) ); ?>"></label>
							<label class="woo-campaign-editor-field"><span><?php esc_html_e( 'End time', 'wc-campaign' ); ?></span><input type="datetime-local" name="campaign_end_at" value="<?php echo esc_attr( $this->formatTimestamp( $end ) ); ?>"></label>
							<label class="woo-campaign-editor-archive"><input type="checkbox" name="campaign_archived" value="1" <?php checked( $archived ); ?>><span><strong><?php esc_html_e( 'Archive campaign', 'wc-campaign' ); ?></strong><small><?php esc_html_e( 'Archived campaigns no longer allow checkout at campaign prices, while historical order attribution is preserved.', 'wc-campaign' ); ?></small></span></label>
						</section>

						<section class="woo-campaign-editor-card">
							<div class="woo-campaign-editor-card-heading compact"><div><span class="woo-campaign-editor-eyebrow"><?php esc_html_e( 'Performance', 'wc-campaign' ); ?></span><h2><?php esc_html_e( 'Campaign performance', 'wc-campaign' ); ?></h2></div></div>
							<div class="woo-campaign-editor-net-sales"><span><?php esc_html_e( 'Net sales', 'wc-campaign' ); ?></span><strong><?php echo wp_kses_post( wc_price( $report['net_sales'] ) ); ?></strong></div>
							<div class="woo-campaign-editor-metrics">
								<div><span><?php esc_html_e( 'Paid orders', 'wc-campaign' ); ?></span><strong><?php echo esc_html( number_format_i18n( $report['orders'] ) ); ?></strong></div>
								<div><span><?php esc_html_e( 'Units', 'wc-campaign' ); ?></span><strong><?php echo esc_html( number_format_i18n( $report['units'] ) ); ?></strong></div>
								<div><span><?php esc_html_e( 'Discount', 'wc-campaign' ); ?></span><strong><?php echo wp_kses_post( wc_price( $report['discount'] ) ); ?></strong></div>
								<div><span><?php esc_html_e( 'Refund', 'wc-campaign' ); ?></span><strong><?php echo wp_kses_post( wc_price( $report['refund'] ) ); ?></strong></div>
							</div>
						</section>
					</aside>
				</div>
			...
			<div class="woo-campaign-product-modal" data-woo-campaign-product-modal hidden>
				...
				<div class="woo-campaign-product-modal-dialog" role="dialog" aria-modal="true" aria-labelledby="woo-campaign-product-modal-title">
					<header><div><span class="woo-campaign-editor-eyebrow"><?php esc_html_e( 'Add product', 'wc-campaign' ); ?></span><h2 id="woo-campaign-product-modal-title"><?php esc_html_e( 'Select WooCommerce products', 'wc-campaign' ); ?></h2></div><button type="button" class="button-link" data-woo-campaign-product-modal-close aria-label="<?php esc_attr_e( 'Close', 'wc-campaign' ); ?>"><span class="dashicons dashicons-no-alt"></span></button></header>
					<div class="woo-campaign-product-modal-body">
						<label class="woo-campaign-editor-field"><span><?php esc_html_e( 'Search products', 'wc-campaign' ); ?></span><select class="wc-product-search" style="width:100%" data-placeholder="<?php esc_attr_e( 'Search products…', 'wc-campaign' ); ?>" data-action="woocommerce_json_search_products" data-allow_clear="true" data-woo-campaign-product-search></select></label>
						<div data-woo-campaign-product-picker-result class="woo-campaign-product-picker-result"></div>
					</div>
					<footer><button type="button" class="button" data-woo-campaign-product-modal-close><?php esc_html_e( 'Cancel', 'wc-campaign' ); ?></button><button type="button" class="button button-primary" data-woo-campaign-product-confirm disabled><?php esc_html_e( 'Add selected', 'wc-campaign' ); ?></button></footer>
				</div>
			</div>
		</div>
		<?php
	...
	public function save(): void {
		...
		try {
			...
			$updated = wp_update_post(
				[
					'ID'           => $campaignId,
					'post_title'   => $title ?: __( 'Untitled Campaign', 'wc-campaign' ),
					'post_name'    => $slug,
			...
			error_log( 'WC Campaign save failed: ' . $error->getMessage() ); // phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_error_log
			wp_die(
				esc_html__( 'Campaign could not be saved. Reload the editor, verify the current values, and try again.', 'wc-campaign' ),
				esc_html__( 'Campaign save failed', 'wc-campaign' ),
	...
	public function ajaxProductDetails(): void {
		...
		if ( ! $product ) {
			wp_send_json_error( [ 'message' => __( 'Product not found.', 'wc-campaign' ) ], 404 );
		}
		...
		if ( ! $normalized ) {
			wp_send_json_error( [ 'message' => __( 'This product cannot be used as a Campaign item.', 'wc-campaign' ) ], 400 );
		}
	...
	private function editorState( int $campaignId ): array {
		...
		return [
			...
			'layouts' => [
				'quick_order' => [ 'label' => 'Quick Order', 'description' => __( 'Expand products and variation quantities for fast multi-item ordering.', 'wc-campaign' ) ],
				'editorial' => [ 'label' => 'Editorial', 'description' => __( 'Prioritize larger imagery and copy for featured products.', 'wc-campaign' ) ],
				'compact_grid' => [ 'label' => 'Compact Grid', 'description' => __( 'Use a compact card grid for add-ons and smaller items.', 'wc-campaign' ) ],
			],
			'i18n' => [
				'untitledSection' => __( 'Untitled product section', 'wc-campaign' ),
				'noProducts' => __( 'This section does not have any products yet.', 'wc-campaign' ),
				'addProduct' => __( 'Add product', 'wc-campaign' ),
				'removeSectionConfirm' => __( 'Delete this section? Its products will move to the first remaining section.', 'wc-campaign' ),
				'duplicateProduct' => __( 'This product or variation is already in the campaign.', 'wc-campaign' ),
				'chooseProductsHelp' => __( 'Choose a simple or variable product from the WooCommerce product catalog.', 'wc-campaign' ),
				'moveUp' => __( 'Move up', 'wc-campaign' ),
				'moveDown' => __( 'Move down', 'wc-campaign' ),
				'deleteSection' => __( 'Delete section', 'wc-campaign' ),
				'sectionTitle' => __( 'Section title', 'wc-campaign' ),
				'sectionTitlePlaceholder' => __( 'For example: Mix and match fragrances', 'wc-campaign' ),
				'sectionDescription' => __( 'Section description', 'wc-campaign' ),
				'sectionDescriptionPlaceholder' => __( 'Describe this product section, offer, or purchase guidance.', 'wc-campaign' ),
				'sectionImage' => __( 'Section image', 'wc-campaign' ),
				'selectImage' => __( 'Select image', 'wc-campaign' ),
				'remove' => __( 'Remove', 'wc-campaign' ),
				'productLayout' => __( 'Product layout', 'wc-campaign' ),
				'productLayoutHelp' => __( 'The layout applies to the entire section to keep its presentation consistent.', 'wc-campaign' ),
				'products' => __( 'Products', 'wc-campaign' ),
				'productAuthorityHelp' => __( 'Campaign items store only campaign price, campaign copy, and display status. WooCommerce remains authoritative for name, image, and stock.', 'wc-campaign' ),
				'displayStatus' => __( 'Display status', 'wc-campaign' ),
				'visible' => __( 'Visible', 'wc-campaign' ),
				'hidden' => __( 'Hidden', 'wc-campaign' ),
				'campaignProductCopy' => __( 'Campaign product copy', 'wc-campaign' ),
				'campaignProductCopyPlaceholder' => __( 'Used only in this campaign and does not change the WooCommerce product description.', 'wc-campaign' ),
				'moveTo' => __( 'Move to', 'wc-campaign' ),
				'searchProductHelp' => __( 'Search for a WooCommerce product. Variable products list their variations.', 'wc-campaign' ),
				'loadingProduct' => __( 'Loading product data…', 'wc-campaign' ),
				'selectAllVariations' => __( 'Select all available variations', 'wc-campaign' ),
				'selectSectionImage' => __( 'Select section image', 'wc-campaign' ),
				'useImage' => __( 'Use this image', 'wc-campaign' ),
			],
		];
	}
	...
	private function decodeArray( string $raw ): array {
		...
		if ( JSON_ERROR_NONE !== json_last_error() || ! is_array( $decoded ) ) {
			wp_die(
				esc_html__( 'The Campaign editor submitted invalid data. Reload the editor and try again.', 'wc-campaign' ),
				esc_html__( 'Invalid Campaign data', 'wc-campaign' ),
	...
	private function staleEditorDie(): never {
		wp_die(
			esc_html__( 'This Campaign was updated in another tab or session. Reload the editor before saving again.', 'wc-campaign' ),
			esc_html__( 'Campaign changed', 'wc-campaign' ),
