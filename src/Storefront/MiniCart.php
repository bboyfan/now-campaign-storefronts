...
final class MiniCart {
		...
		<div class="woo-campaign-mini-cart<?php echo $snapshot['count'] > 0 ? ' has-items' : ' is-empty'; ?>" data-woo-campaign-mini-cart>
			...
			<div class="woo-campaign-mini-cart-panel" data-woo-campaign-mini-panel hidden>
				<div class="woo-campaign-mini-cart-header">
					<div>
						<span class="woo-campaign-section-eyebrow"><?php esc_html_e( 'Cart summary', 'now-campaign-storefronts' ); ?></span>
						<h3><?php esc_html_e( 'Ready when you are', 'now-campaign-storefronts' ); ?></h3>
					</div>
					<button type="button" class="woo-campaign-mini-cart-close" data-woo-campaign-mini-close aria-label="<?php esc_attr_e( 'Close cart', 'now-campaign-storefronts' ); ?>"><span class="dashicons dashicons-no-alt"></span></button>
				</div>
				...
				<div class="woo-campaign-mini-cart-footer">
					<div class="woo-campaign-mini-cart-summary">
						...
						<div class="woo-campaign-mini-cart-discount-row"><span><?php esc_html_e( 'Discount', 'now-campaign-storefronts' ); ?></span><span data-woo-campaign-cart-discount><?php echo wp_kses_post( $snapshot['discount'] ); ?></span></div>
						<div class="woo-campaign-mini-cart-total"><span><?php esc_html_e( 'Total', 'now-campaign-storefronts' ); ?></span><strong data-woo-campaign-cart-total><?php echo wp_kses_post( $snapshot['total'] ); ?></strong></div>
					</div>
	...
	private function renderItems( array $items ): string {
		if ( ! $items ) {
			return '<div class="woo-campaign-cart-empty"><span class="dashicons dashicons-cart"></span><strong>' . esc_html__( 'Your cart is empty', 'now-campaign-storefronts' ) . '</strong><p>' . esc_html__( 'Add a campaign item to get started.', 'now-campaign-storefronts' ) . '</p></div>';
		}
		$html = '';
		foreach ( $items as $item ) {
			...
			if ( $item['campaign'] ) {
				$html .= '<span class="woo-campaign-badge">' . esc_html__( 'Campaign', 'now-campaign-storefronts' ) . '</span>';
			}
			$html .= '</div>';
			$html .= '<div class="woo-campaign-mini-cart-item-controls">';
			$html .= '<div class="woo-campaign-cart-quantity"><button type="button" data-woo-campaign-cart-step="-1" aria-label="' . esc_attr__( 'Decrease quantity', 'now-campaign-storefronts' ) . '">−</button><input type="number" min="0" step="1" value="' . esc_attr( (string) $item['quantity'] ) . '" data-woo-campaign-cart-qty aria-label="' . esc_attr__( 'Quantity', 'now-campaign-storefronts' ) . '"><button type="button" data-woo-campaign-cart-step="1" aria-label="' . esc_attr__( 'Increase quantity', 'now-campaign-storefronts' ) . '">+</button></div>';
			$html .= '<button type="button" class="woo-campaign-remove-link" data-woo-campaign-cart-remove>' . esc_html__( 'Remove', 'now-campaign-storefronts' ) . '</button></div></div>';
